...
public class Files {
     ...
     * @param file 要写入的文件
     */
    public static void writeWithBuffer(byte[] data, File file) {
    ...
    public static byte[] readWithBuffer(File file) {
        if (file == null || !file.exists()) return null;
        if (file.isDirectory()) return null; //如果是个目录也是没法读的.
        ...
        try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(file)) ) {
            //分配文件长度的字节数组.
            byte[] bytes = new byte[(int) file.length()];
            bis.read(bytes); //把文件读到这个数组中. read 的返回值 是 读了多少字节的 有效长度.
            return bytes;
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
     ...
     * @param dest
     */
    public static void copyWithBuffer(File src, File dest) {
        if (src == null || dest == null) return;
        if (dest.exists()) return; //目标文件存在也不拷贝
        if (src.isDirectory()) return; //暂时 只拷贝 文件.
        //还要考虑 目标文件中父路径问题.
        ...
        try (
                // ✅ ps:拷贝 任何文件 就不能使用 字符流. 字符流 读的时候 需要编码.
                // 带缓冲的字节流 .
            ...
            byte[] caches = new byte[8192];//8kb 的字节数组 当个缓存小桶
            int len;
            while ((len = bis.read(caches)) != -1) {
                //注意 只要是同一个fos对象,写的时候不管写多少都会追加到后面.
                bos.write(caches, 0, len);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
